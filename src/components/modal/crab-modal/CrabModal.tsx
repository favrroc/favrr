...
import { setShowCrabModalAction } from "../../../core/store/slices/modalSlice";
...
const CrabModal = () => {
  ...
  const avatarPreviewUrl = hasImageFileFormatError ? WrongImagePlaceholderImage().props.src
  ...
  const onClose = () => {
    ...
    dispatch(setShowCrabModalAction(false));
  };
  ...
  const postNextStep = async (data: boolean) => {
    // sessionStorage.setItem("hasNextStep", "true");
  ...
  const titleDescription = useMemo(() => {
    ...
    // When Fav battles are implemented, remove the comment // above and remove the if below.
    if (league === League.Crab) {
        ...
        <>
          ...
          <h2 style={{ display: "flex" }}>
            <span className="report-title">
              <ReactMarkdown linkTarget="_blank">{titleDescription[0]}</ReactMarkdown>
            </span>
          </h2>
          ...
          {league !== League.Microbe ? (
            <div className="content">
              ...
              {/* Uncomment the button below once Battles are implemented */}
